...
export type CardData = {
  ...
  title: string;
  icon: LucideIcon;
  currentWeekValue: number | string;
  lastWeekValue?: number | string;
  isDate?: boolean;
  isCurrency?: boolean;
};
        ...
        <Card key={item.id} className="gap-2 py-4 overflow-hidden shadow-none">
          ...
          <CardContent>
            <div className="text-3xl">
              {item.isCurrency
                ? formatCurrency(item.currentWeekValue as number)
                : item.currentWeekValue}
            </div>
          </CardContent>
          <CardFooter className="text-xs">
            {item.lastWeekValue !== undefined && (
              <div className="flex items-center text-muted-foreground">
                {(item.currentWeekValue as number) >
                (item.lastWeekValue as number) ? (
